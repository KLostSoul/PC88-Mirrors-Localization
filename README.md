...
- 본문은 16행 프로필과 `&H50*16` 행 이동을 사용해 상단 잘림 없이 출력된다. UI 설명·로딩 문구는 아직 영문 11행 간격이 남아 있어 한글 16행 글리프 출력 시 줄 겹침이 발생하며, 별도 조정 대상이다.
- 시험 산출물은 `korean_mirrors_tools/Temp/english_menu_no0_no1_test/`에서 관리한다. 이전 `v2` 산출물은 삭제했다.

## 다음 작업

1. UI 설명·로딩 문구의 16행 출력 프로필·줄 간격·창 높이를 조정한다.
2. 대조표를 기준으로 한국어 번역표를 작성한다.
3. `stringsImport.csv`의 디스크·스크립트·BASIC 라인·문자열 번호·원문 좌표를 유지하면서 번역 열을 채운다.
4. 실제 번역문에서 고유 한글 글리프를 집계하고 최종 토큰표를 만든다.
5. 한글 글리프 PNG와 폰트 데이터를 생성하고 기존 VWF 경로에 적용한다.
6. 번역된 BASIC의 크기, 2바이트 토큰 처리, CD Track 2 배치와 CloneCD 생성물을 정적으로 검증한다.
