...
이 문서는 영문 패치 소스에 포함된 **Ruby 빌드 도구**와 데이터 CSV, BASIC/ASM 소스를 기준으로 작성한 정적 구조 분석표다. `reference/mirrors_tools/Ruby/`가 영문 패치의 기준 구현이며, 현재 작업 폴더의 Python 포팅본은 한글 패치 빌드에 사용할 별도 활성 구현이다.
...
- 따라서 현재 영문 빌드 범위에서는 Python 포트가 Ruby 2.7.4 결과와 바이트 단위로 호환된다.
