...
현재 뱅크 RAM에서 VWF 코드가 `0x0000~0x0FFF`를 사용하고, 세 폰트 영역은 `0x1000~0x3FFF`의 `0x3000`바이트를 사용한다. 여기서 `0x3000`은 세 폰트 영역을 합친 주소 공간의 크기이며, `copyFnt` 한 번의 복사량이 아니다. 기존 `copyFnt`의 한 번당 복사량은 `0x2000`바이트다.
