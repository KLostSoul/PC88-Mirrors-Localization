...
- 편집기 저장 또는 정식 빌드 시작 시 새 음절을 자동 감지해 토큰표를 재생성한다.
...
- 번역 CSV와 편집기에는 일반적인 `"`를 그대로 입력한다. 사용자가 `\"`를 직접 입력하지 않는다.
...
- 메뉴 하드코딩 문구 14개를 `hardcoded_strings.csv`와 편집기에 연결했고, 편집기에서 수정한 값이 용량 검사와 전체 빌드에 반영되도록 했다.
...
- `Composite_16x16/source`의 글리프 원본·생성기·편집기
- 설계·분석·검수 문서
- 영문 패치의 공개 소스와 출처 기록
