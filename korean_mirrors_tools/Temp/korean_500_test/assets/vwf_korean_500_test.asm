...
_nextSymbol:    ld      hl,(vStrAddr)
                ...
                ld      (vStrAddr),hl
                ld      a,e
                ; Korean test token: E0/E1/E2 plus a control-safe trail byte.
                ; The existing length is a byte count plus one, so consume the
                ; second token byte here as well.  Existing one-byte controls
                ; (including 0Dh) continue through their original path.
                cp      0xe0
...
_drawSymbol:
                ...
                out     (0x32),a        ; '2'
                push    bc
                call    convertKoreanTestCharAddr
...
; Returns address to a Korean test glyph in DE for the safe 500-token table.
; All other values retain the existing ASCII lookup for unchanged controls
; or any unintended printable ASCII that reaches this test build.
convertKoreanTestCharAddr:
                ld      a,d
                cp      0xe0
                jr      c,convertASCII_toCharAddr
                cp      0xe3
                jr      nc,convertASCII_toCharAddr
                ld      a,e
                cp      0x40
                jr      c,_koreanTestGlyphZero
                cp      0x7f
                jr      c,_koreanTestTrailLow
                cp      0x80
                jr      c,_koreanTestGlyphZero
                cp      0xfd
                jr      nc,_koreanTestGlyphZero
                sub     0x41
                jr      _koreanTestTrailReady
_koreanTestIndexValid:
_koreanTestTrailLow:
                sub     0x40
_koreanTestTrailReady:
                ld      l,a
                ld      h,0
                ld      a,d
                sub     0xe0
                ld      b,a
                ld      de,0x00bc
_koreanTestLeadOffset:
                ld      a,b
                or      a
                jr      z,_koreanTestAddress
                add     hl,de
                dec     b
                jr      _koreanTestLeadOffset
_koreanTestAddress:
                add     hl,hl
                add     hl,hl
                add     hl,hl
                ...
                ex      de,hl
                ret
_koreanTestGlyphZero:
                ld      de,0x1000
                ret

; 8 pixels = existing renderer's right-margin value 5 plus its fixed 3.
getKoreanTestRightMargin:
                ld      a,0x05
                ld      (vRightMargin),a
                ret

; Returns address to ASCII char in DE
