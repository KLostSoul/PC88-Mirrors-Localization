1 'SAVE "MENU.BAS"
10 CLEAR ,&H8FFF:DEFINT A-Z:CLS:POKE &HE3FF,50:POKE &HD456,1:POKE &HE3FE,0
...
1200 BM$="Start new game":CMD WIDTH YY,&H80,7:CMD KANJI BM$:YY=YY+&H50*16
1210 BM$="Load saved game":CMD WIDTH YY,&H80,7:CMD KANJI BM$:YY=YY+&H50*16
1214 BM$="Clear all saves":CMD WIDTH YY,&H80,7:CMD KANJI BM$:YY=YY+&H50*16
1215 BM$="Settings":CMD WIDTH YY,&H80,7:CMD KANJI BM$:YY=YY+&H50*16
...
3899 '
3900 '
3910 POKE &H9089,D:CMD SET "B:":GOSUB 5600:COLOR=(7,7):CMD SCREEN 0:CMD RUN E$
3999 '
4000 CMD SCREEN 0:BM$="Once you are ready, please press Return.":CMD WIDTH &HF2B0,&H60,7:CMD KANJI BM$
4001 BM$="Press [ESC] to return to the menu.":CMD WIDTH &HF7B0,&H60,2:CMD KANJI BM$:GOSUB 5620
...
4501 BM$="Press [ESC] to return to the menu.":CMD WIDTH &HF7B0,&H60,2:CMD KANJI BM$:GOSUB 5620:RETURN
4999 '
5400 '
5420 LINE(258,76+CN*16)-(530,91+CN*16),7,BF,XOR:CN2=CN
...
5440 LINE(258,76+CN2*16)-(530,91+CN2*16),7,BF,XOR
...
10100 LINE(368+X1*60,72)-(399+X1*60,89),7,BF,XOR:X2=X1:RETURN
10110 LINE(368+X2*60,72)-(399+X2*60,89),7,BF,XOR:RETURN
10119 '
...
11100 LINE(372+V1*32,72+20)-(385+V1*32,89+20),7,BF,XOR:V2=V1:RETURN
11110 LINE(372+V2*32,72+20)-(385+V2*32,89+20),7,BF,XOR:RETURN
11200 '
...
11230 IF Z1=0 THEN Z3=336 :ELSE IF Z1=1 THEN Z3=392 :ELSE Z3=444
11231 LINE(Z3,72+40)-(Z3+47,89+40),7,BF,XOR:Z2=Z1:RETURN
11240 IF Z2=0 THEN Z3=336 :ELSE IF Z2=1 THEN Z3=392 :ELSE Z3=444
11241 LINE(Z3,72+40)-(Z3+47,89+40),7,BF,XOR:RETURN
12000 IF DX>0 THEN DX=DX-1
12005 RETURN
12010 IF DX<CM-1 THEN DX=DX+1
12015 RETURN
